...
#: How the water is drawn: the rivers, and the springs they come out of. Blue
#: because that is what water is on a map, and brighter than the lake colour so
#: a river crossing shallow ground is still a line.
...
class MapScene:
    ...
    def contours(self) -> Group | None:
        ...
        if self._contours is None:
            ...
            extent = self.project.landscape.extent
            found = contours_of(self.world().natural(), extent=extent,
    ...
    def start_mark(self, metres_per_pixel: float) -> Shape | None:
        ...
        half = self.project.world().circuit().profile.total_width \
    ...
    def _river_shape(self, channel: Any) -> Shape | None:
        """One river, draped on the ground it cut."""
        line = np.asarray(channel.points, dtype='d').reshape(-1, 2)
        if len(line) < 2:
            return None
        y = np.asarray(self.world().height_fn()(line[:, 0], line[:, 1]),
                       dtype='d') + WATER_LIFT
        points = np.stack([line[:, 0], y, line[:, 1]], axis=1)
        return Shape(
            geometry=IndexedLineSet(coord=Coordinate(point=points),
                                    coordIndex=[*range(len(points)), -1],
                                    color=None),
            appearance=Appearance(material=PBRMaterial(
