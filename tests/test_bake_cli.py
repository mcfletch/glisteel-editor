"""``glisteel-bake`` from the outside: what the command line does and refuses."""

import os

import pytest

from glisteel_editor import bake
...
class TestBakingFromTheCommandLine:
    ...
    def test_it_says_how_to_drive_what_it_baked(self, tmp_path, capsys) -> None:
        _run(tmp_path)
        assert 'glisteel' in capsys.readouterr().out
